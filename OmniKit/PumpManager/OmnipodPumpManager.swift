...
public class OmnipodPumpManager: RileyLinkPumpManager, PumpManager {
    ...
    public init(state: OmnipodPumpManagerState, rileyLinkDeviceProvider: RileyLinkDeviceProvider, rileyLinkConnectionManager: RileyLinkConnectionManager? = nil) {
        ...
        if let podState = state.podState {
            self.podComms = PodComms(podState: podState, delegate: self, messageTransportDelegate: self)
        } else {
            self.podComms = nil
        }
    }
    ...
    public static func jumpStartPod(address: UInt32, lot: UInt32, tid: UInt32) -> OmnipodPumpManager {
        ...
        let podState = PodState(address: address, activatedAt: Date(), expiresAt: Date().addingTimeInterval(.days(3)), piVersion: "jumpstarted", pmVersion: "jumpstarted", lot: lot, tid: tid)
        let state = OmnipodPumpManagerState(podState: podState, timeZone: TimeZone.currentFixed, rileyLinkConnectionManagerState: connectionManager.state)
    ...
    public func pair(completion: @escaping (Error?) -> Void) {
        queue.async {
            ...
            PodComms.pair(using: deviceSelector, timeZone: .currentFixed, messageTransportDelegate: self) { (result) in
                switch(result) {
    ...
    public func insertCannula(schedule: BasalSchedule, completion: @escaping (Error?) -> Void) {
        queue.async {
            guard let podComms = self.podComms else
            {
                completion(OmnipodPumpManagerError.noPodPaired)
        ...
        queue.async {
            ...
            podComms.runSession(withName: "Resume", using: rileyLinkSelector) { (result) in
                ...
                do {
                    ...
                    let status = try session.resumeBasal(scheduleOffset: scheduleOffset)
                    completion(PumpManagerResult.success(status.deliveryStatus != .suspended))
                } catch (let error) {
                    completion(PumpManagerResult.failure(error))
                }
        ...
        queue.async {
            ...
            podComms.runSession(withName: "Bolus", using: rileyLinkSelector) { (result) in
                ...
                if podStatus.deliveryStatus == .suspended {
                    do {
                        let scheduleOffset = self.state.timeZone.scheduleOffset(forDate: Date())
                        podStatus = try session.resumeBasal(scheduleOffset: scheduleOffset)
                    } catch let error {
                        completion(SetBolusError.certain(error as? PodCommsError ?? PodCommsError.commsError(error: error)))
                        return
                    }
    ...
    public func setTime(completion: @escaping (Error?) -> Void) {
        ...
        queue.async {
            ...
            podComms.runSession(withName: "Set time zone", using: rileyLinkSelector) { (result) in
                ...
                case .success(let session):
                    do {
                        let _ = try session.setTime(timeZone: timeZone, date: Date())
...
extension OmnipodPumpManager: MessageTransportDelegate {
